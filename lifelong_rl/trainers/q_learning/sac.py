...
class SACTrainer(TorchTrainer):
    ...
    def __init__(
        ...
        # dasp
        self.dasp_model = dasp_model
        self.dasp_lambda = dasp_lambda
        self.dasp_optimizer = torch.optim.Adam(self.dasp_model.parameters(), lr=1e-4, weight_decay=0)
    ...
    def train_from_torch(self, batch, indices, use_dasp=False, use_dasp_pi=False):
        ...
        if self.policy_smooth_eps > 0 and self.policy_smooth_reg > 0:
            ...
            noised_actions, noised_policy_mean, noised_policy_log_std, _, *_ = self.policy(noised_obs,reparameterize=True)
            ...
            if use_dasp_pi:
                #self.dasp_model.eval()
                _, _, _, noised_obs_dasp, _ = self._get_noised_obs(obs, actions, self.policy_smooth_eps)
                # noised_actions_dasp, _, _, _, *_ = self.policy(noised_obs_dasp,reparameterize=True)
                R = self.dasp_model.R_loss(noised_obs_dasp, noised_actions)
                policy_loss += self.dasp_lambda * R.mean()
                if self._need_to_update_eval_statistics:
                    self.eval_statistics['Dasp'] = ptu.get_numpy(R.mean()) * self.dasp_lambda
        ...
        # q_target_detach -= self.dasp_lambda * R
        ...
        if self.q_smooth_eps > 0 and self.q_smooth_reg > 0:
            ...
            qs_temp = qs_pred.repeat(1, 1, size).reshape(self.num_qs, -1, 1)

            if use_dasp:
                R = self.dasp_model.R_loss(noised_obs, actions.reshape(-1, 1, A).repeat(1, size, 1).reshape(-1, A))

                qs_temp -= self.dasp_lambda * R.repeat((self.num_qs, 1)).unsqueeze(-1).detach()
                if self._need_to_update_eval_statistics:
                    self.eval_statistics['Dasp_q'] = ptu.get_numpy(R.mean()) * self.dasp_lambda

            diff = noised_qs_pred - qs_temp
        ...
        self.dasp_model.train()
        self.dasp_optimizer.zero_grad()
        vae_loss = self.dasp_model.train_vae(obs, actions, next_obs)
        vae_loss.backward()
        self.dasp_optimizer.step()
