...
import abc
import numpy as np
from lifelong_rl.util.eval_util import get_average_returns
...
class PerformanceHistory:
    def __init__(self, max_size=10):
        if max_size <= 0:
            raise ValueError("max_size must be positive int")
        self.history = deque(maxlen=max_size)

    def add(self, performance_value):
        self.history.append(performance_value)
    ...
    def __str__(self):
        return f"PerformanceHistory(size={len(self)}, max_size={self.history.maxlen}, mean={self.mean():.4f})"
...
class OfflineRLAlgorithm(object, metaclass=abc.ABCMeta):
    def __init__(
        ...
        self.eval_scores_res = PerformanceHistory(max_size=15)
        self.max_eval_scores_res = 0
        ...
        for epoch in gt.timed_for(
            ...
            for _ in range(self.num_train_loops_per_epoch):
                for _ in range(self.num_trains_per_train_loop):
                    ...
                    if self.eval_env.spec.id == 'walker2d-medium-v2':
                        self.trainer.train(train_data, indices, use_dasp=True, use_dasp_pi=False)
                    else:
                        self.trainer.train(train_data, indices, use_dasp=True, use_dasp_pi=True)
    ...
    def _log_stats(self, epoch):
        ...
        if self.num_eval_steps_per_epoch > 0:
            ...
            if hasattr(self.eval_env, 'get_diagnostics'):
                statistics = logger.record_dict(
                    self.eval_env.get_diagnostics(eval_paths),
                    prefix='evaluation/',
                )
                self.eval_scores_res.add(get_average_returns(eval_paths))
                avg_res = self.eval_scores_res.mean()
                if avg_res >= self.max_eval_scores_res:
                    snapshot = self._get_snapshot()
                    _snapshot_dir = logger.get_snapshot_dir()
                    file_name = osp.join(_snapshot_dir, 'offline_best.pkl')
                    torch.save(snapshot, file_name)
                    self.max_eval_scores_res = avg_res

            statistics = eval_util.get_generic_path_information(eval_paths)
            logger.record_dict(
                statistics,
                prefix="evaluation/",
            )
